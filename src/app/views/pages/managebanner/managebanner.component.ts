...
import { BannerpopupComponent } from './bannerpopup/bannerpopup.component';
...
export class ManagebannerComponent implements OnInit {
  ...
  getItems() {
    this.getItemSub = this.service.getAllHowItworks()
      .subscribe(data => {
        this.items = data;
      })
  }
  ...
  openPopUp(data: any = {}, isNew?,_id:any="") {
    ...
    let dialogRef: MatDialogRef<any> = this.dialog.open(BannerpopupComponent, {
      width: '720px',
      height:'500px',
      disableClose: true,
    ...
    dialogRef.afterClosed()
      .subscribe(res => {
        ...
        if (isNew) {
          ...
          res.append("save","yes");
          this.service.saveHowItworks(res)
            .subscribe(data => {
  ...
  deleteItem(row) {
    ...
    this.confirmService.confirm({message: `Delete ${row.name}?`})
      .subscribe(res => {
        if (res) {
          this.loader.open();
          this.service.deleteHowItworks({categoryid:row})
            .subscribe(data => {
              this.items = data["data"];
              this.loader.close();
              this.snack.open(data["message"], 'OK', { duration: 4000 })
